...
export class ResilientSelectorEngine {
   ...
   */
  async discoverWithAI(purpose: string): Promise<SelectorResult[]> {
   ...
   */
  private async findSemanticElements(purpose: string): Promise<SelectorResult[]> {
    ...
    for (const selector of selectors) {
      if (this.failedSelectors.has(selector)) continue;
   ...
   */
  private async findFormElements(purpose: string): Promise<SelectorResult[]> {
    const strategies: Record<string, Array<{ selector: string; attrs: string[] }>> = {
      search: [
    ...
    for (const config of configs) {
      if (this.failedSelectors.has(config.selector)) continue;
   ...
   */
  private async findVisualElements(purpose: string): Promise<SelectorResult[]> {
    const results: SelectorResult[] = [];
   ...
   */
  private async findHeuristicElements(purpose: string): Promise<SelectorResult[]> {
    const patterns: Record<string, Array<{ pattern: string; attrs: string[]; weight: number }>> = {
      search: [
  ...
  private async getPageContext(): Promise<{
    ...
    url: string;
    structure: string;
    inputs: Array<{ type: string; placeholder: string; location: string }>;
    buttons: string[];
  }> {
    if (!this.page) throw new Error('Page not initialized');
    ...
    return await this.page.evaluate(() => {
      ...
      const buttons = Array.from(document.querySelectorAll('button')).map(b => b.textContent?.trim()).filter(Boolean) as string[];

      const structure = Array.from(new Set(
   ...
   */
  private calculateSemanticConfidence(purpose: string, selector: string, count: number): number {
    let confidence = 0.5;
   ...
   */
  private async checkRelevantAttributes(element: any, purpose: string): Promise<boolean> {
    try {
   ...
   */
  async validateSelector(purpose: string, selector: string): Promise<boolean> {
